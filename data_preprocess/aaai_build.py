...
def generate_more_negative_AAAI():
    ...
    root_path = '/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/dataset/gossip'
    dataset_name = 'gossip'
    xlsx = f'{root_path}/origin_do_not_modify/gossip_train_reload.xlsx'
    ...
    ambiguity_excel = f'{root_path}/origin_do_not_modify/gossip_train_more_negative.xlsx'
...
def reload_xlsxs_AAAI(dataset_name=['gossip']):
    ...
    hashing_not_allowed, ban_images = {}, os.listdir(f'{root_path}/Images/ban_images')
    for ban_image in ban_images:
        hash = photohash.average_hash(f'{root_path}/Images/ban_images/' + ban_image)
        hashing_not_allowed[ban_image] = hash

    for dataset in dataset_name:
        for sub_folder in sub_folders:
            xlsx = "{}/origin_do_not_modify/{}_{}.xlsx".format(root_path,dataset,sub_folder)
            sheet_rumor, rows_rumor = get_workbook(xlsx)
            records_list = []
            ...
            df = pd.DataFrame(records_list)
            df.to_excel(f'{root_path}/origin_do_not_modify/{dataset}_{sub_folder}_reload.xlsx')
...
def create_xlsx():
    root_path = '/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/dataset/gossip'
    with open(f'{root_path}/gossip_train.csv',mode='r') as f:
        result = []
        st1 = ''
        for i in f:
            if ',content,image,label' in i:
                continue
            i = i.replace('\n',' ')
            st1 += i
            if '.jpg,' in i:
        ...
        for index,i in enumerate(result):
            ...
            print(index, i)
            # 移除序号
            rm_start_num = str(index)+','
            # print(len(rm_start_num))
            i = i[len(rm_start_num):]
            i_ls = i.split(',')
            # print(i_ls[-2],i_ls[-1])
            i_other = f',{i_ls[-2]},{i_ls[-1]}'
            i_content = i.replace(i_other,'')
            print(i_content)
            records['content'] = i_content
            records['image'] = i_ls[-2]
            records['label'] = i_ls[-1]
            result_xlsx.append(records)

        df = pd.DataFrame(result_xlsx)
        df.to_excel(f'{root_path}/origin_do_not_modify/gossip_train.xlsx')
