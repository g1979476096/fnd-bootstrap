...
def generate_ambiguity():
    ...
    xlsxs = [f"{Weibo_21_path}/origin_do_not_modify/train_datasets.xlsx",
             f"{Weibo_21_path}/origin_do_not_modify/test_datasets.xlsx"]
    ...
    for xlsx in xlsxs:
        ...
        while (num_real_count<THRESH or num_fake_count<THRESH) and len(valid_list_negative_text)>0 and len(valid_list_positive)>0:  # title label content image B C E F
            ...
            for new_image in new_images:
                is_found_image = cv2.imread(f"{Weibo_21_path}/" + new_image)
    ...
    ambiguity_excel = f"{Weibo_21_path}/origin_do_not_modify/{dataset_name}_train_ambiguity_new.xlsx"
...
def download_images_and_load():
    ...
    # Num of news: 9128
    df.to_excel(f'{Weibo_21_path}/origin_do_not_modify/all_datasets_Weibo21_origin.xlsx')
...
def reload_Weibo21():
    ...
    folder1, folder2 = f'{Weibo_21_path}/nonrumor_images', f'{Weibo_21_path}/rumor_images'
    images1, images2 = set(os.listdir(folder1)), set(os.listdir(folder2))
    # yan: 把所有图片去重后集合在一起
    images_set = images1.union(images2)
    news_xlsxs = []
    news_xlsxs.append(f'{Weibo_21_path}/origin_do_not_modify/all_datasets_Weibo21_origin.xlsx')
            ...
            if image_names is not None and len(image_names) != 0:
                ...
                for image_name in image_name_list:
                    ...
                    if short_name in images_set:
                        ...
                        is_found_image = cv2.imread(f'{Weibo_21_path}/' + new_image) is not None
    ...
    filename = f'{Weibo_21_path}/origin_do_not_modify/train_datasets.xlsx'
    # filename = './dataset/Weibo_21/origin_do_not_modify/train_datasets.xlsx'
    df = pd.DataFrame(train_list)
    df.to_excel(filename)
    print(f"File saved at {filename}")
    filename = f'{Weibo_21_path}/origin_do_not_modify/val_datasets.xlsx'
    # filename = './dataset/Weibo_21/origin_do_not_modify/val_datasets.xlsx'
    df = pd.DataFrame(val_list)
    df.to_excel(filename)
    print(f"File saved at {filename}")
    filename = f'{Weibo_21_path}/origin_do_not_modify/test_datasets.xlsx'
    ...
    # length 17 CV_Error 0 No_valid 2952
    # Train_set 4928 Test_set 615 Val_set 616


if __name__ == '__main__':
    generate_ambiguity()
    # download_images_and_load()
    # reload_Weibo21()