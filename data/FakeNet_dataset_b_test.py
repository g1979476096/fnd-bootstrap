...
class FakeNet_dataset(data.Dataset):
    # /home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/dataset/gossip
    # /home/groupshare/AAAI_dataset
    def __init__(
            self, root_path='/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/dataset/gossip', dataset='gossip',
        ...
        workbook_name = self.root_path + '/origin_do_not_modify/{}_{}_reload.xlsx'.format(self.dataset_name, 'train' if self.is_train else 'test')
        ...
        if self.dataset_name == 'gossip' and self.is_train and self.with_ambiguity:
            # wb = openpyxl.load_workbook(self.root_path + '/{}/{}_{}.xlsx'.format(self.dataset_name, self.dataset_name, 'train_ambiguity'))
            wb = openpyxl.load_workbook(self.root_path + '/origin_do_not_modify/{}_train_more_negative.xlsx'.format(self.dataset_name))
            sheetnames = wb.sheetnames
            sheet = wb[sheetnames[0]]
            rows = sheet.max_row
            for i in tqdm(range(2, rows + 1)):
    ...
    def __getitem__(self, index):
        ...
        while not find_path:
            ...
            # 此处设置解释之后的图像文本返回值
            inter_filepath = images.split('.')[0]
            # print(inter_filepath)
            if self.is_train :
                # interpretability_img = torch.zeros(3, 224, 224)
                # interpretability_text = torch.zeros(197, 300)
                try:
                    # interp_a
                    interpretability_img = torch.load(f'/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/dataset/interpreability/gossip/interp_f/train/image/{inter_filepath}.pt')
                    interpretability_text = torch.load(f'/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/dataset/interpreability/gossip/interp_f/train/tst/{inter_filepath}.pt')
                    interpretability_img = transforms.Resize([224, 224])(interpretability_img).float()
                except Exception:
                    # print("[Exception] load train interpretability tensor error. Using a zero-matrix instead")
                    interpretability_img = torch.zeros(3, 224, 224)
                    interpretability_text = torch.zeros(197, 300)
            else:
                # interpretability_img = torch.zeros(3, 224, 224)
                # interpretability_text = torch.zeros(197, 300)
                try:
                    interpretability_img = torch.load(f'/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/dataset/interpreability/gossip/interp_f/test/image/{inter_filepath}.pt')
                    interpretability_text = torch.load(f'/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/dataset/interpreability/gossip/interp_f/test/tst/{inter_filepath}.pt')
                    interpretability_img = transforms.Resize([224, 224])(interpretability_img).float()
                except Exception:
                    # print("[Exception] load test interpretability tensor error. Using a zero-matrix instead")
                    interpretability_img = torch.zeros(3, 224, 224)
                    interpretability_text = torch.zeros(197, 300)
            # 注意： 收集MixSet的时候，把真新闻标记为1，因此最好把它反过来
        ...
        # print(img_GT.shape,'yan'*20) torch.Size([3, 224, 224])
        ...
        if not self.with_ambiguity:
            return (content, img_GT, img_GT, label, 0), (GT_path, interpretability_img,interpretability_text)
        else:
            return (content, img_GT, img_GT, label, 0), (GT_path, interpretability_img, interpretability_text), (content_ambiguity, img_ambiguity, label_ambiguity)
