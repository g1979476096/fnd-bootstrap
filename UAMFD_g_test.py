...
# yan: input_ids 就是一连串 token 在字典中的对应id。形状为 (batch_size, sequence_length)。
...
def main(args):
    ...
    # setting['checkpoint_path'] = '/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/output_file/gossip_dis/test_xai_1/41_124_89.pkl'
    print('loading checkpoint from {}'.format(setting['checkpoint_path']))
    setting['train_dataname'] = args.train_dataset
    setting['val_dataname'] = args.test_dataset  # yan:validate 验证数据集
    ...
    setting['with_ambiguity'] = True   # yan:应该是论文里设置 D' 数据集，进行模态一致性训练
    ...
    if setting['network_arch'] == 'UAMFDv2':
        from models.UAMFDv2_Net_g_test import UAMFD_Net
    ...
    # yan: loss_bce(input, target)  input为样本数量，即pred 为任意维度； target为正确数据 即label ，维度同input
    ...
    # yan:T_max指cosin函数经过多少次更新完成1/2个周期
    # warmup是针对学习率learning rate优化的一种策略
    ...
    for epoch in range(custom_num_epochs):
        ...
        if setting['val'] == 0:
            ...
            # print(len(train_dataset), len(train_loader),'len')
            # 9759 1219 len
            ...
            for i, items in enumerate(train_loader):
               ...
               (input_ids1, attention_mask1, token_type_ids1), (image1, labels1, sents1)
                        ...
                        input_ids, attention_mask, token_type_ids, image, image_aug, labels, category, interpretability_img, interpretability_text = \
                            ...
                            to_var(image), to_var(image_aug), to_var(labels), to_var(category), to_var(interpretability_img), to_var(interpretability_text)
                    ...
                    if setting['with_ambiguity']:
                        ...
                        aux_output, *_ = model(input_ids=input_ids1,
                                          ...
                                          calc_ambiguity=True,
                                          interpretability_img_s=interpretability_img,
                                          interpretability_text_s=interpretability_text
                                         )
                        loss_ambiguity += criterion(aux_output,labels1.float().unsqueeze(1))
                        # loss_ambiguity 是 tensor类型   loss_ambiguity.item() 获取内部数值
                    ...
                    # 0513
                    # train_mode = 1
                    mix_output_1, mm_only_output, image_only_output, text_only_output, aux_output, mi_loss = model(input_ids=input_ids,
                    ...
                    # 20240429 加权BCE  label:[1,0] => weight:[0.4,1.6]
                    # # Thresh 设为 0.5
                    ...
                    # weight = torch.where(labels>0.5,0.4,1.6)
                    # criterion_new = nn.BCEWithLogitsLoss(weight=weight).cuda()
                    #
                    # loss_CE = criterion_new(mix_output, labels)
                    # loss_CE_image = criterion_new(image_only_output, labels)
                    # loss_CE_text = criterion_new(text_only_output, labels)
                    # loss_CE_mm = criterion_new(mm_only_output, labels)
                    # 0429
                    ...
                    loss_CE_mm = criterion(mm_only_output, labels)

                    loss_MI  = mi_loss
                    # loss_MI = mi_loss if mi_loss > 0 else torch.tensor(0)
                    # loss_CE_vgg = criterion(vgg_only_output, labels)
                    # loss_single_modal = (loss_CE_vgg+loss_CE_text+loss_CE_image)/3
                    loss_single_modal = (loss_CE_text+loss_CE_image+loss_CE_mm)/3
                    # 设置正则项  bsz为 8
                    num_class = 2  # 类别数
                    mr_weight_kld = 0.1  #权重系数
                    reg_loss = mr_weight_kld * torch.sum((-torch.log(torch.sigmoid(mix_output[2:8])))/num_class*mr_weight_kld)
                    # reg_loss = mr_weight_kld * torch.sum((-torch.log(torch.sigmoid(mix_output_2[2:8])))/num_class*mr_weight_kld)


                    if torch.isinf(reg_loss) or torch.isnan(reg_loss):
                        reg_loss = torch.tensor(0)
                    #
                    loss = loss_CE+2.0*loss_ambiguity+1.0*loss_single_modal+ reg_loss
                           # +0.01*loss_MI + reg_loss
                    # + reg_loss
                    # +1.0*loss_MI
                    # yan:
                    # print('=' * 20)
                    # print(mix_output, labels)  shape [8,1]
                    # print(loss_CE,loss_CE.shape)
                    # print(mi_loss,mi_loss.shape) tensor(54.6749,device= 'cuda:0', grad_fn=<SumBackward0>) torch.Size([])

                    # print(loss_single_modal,loss_single_modal.shape)
                    # print(loss, loss.shape)
                    # tensor(0.6810, device='cuda:0',
                    #        grad_fn= < BinaryCrossEntropyWithLogitsBackward0 >) torch.Size([])
                    # tensor(0.7693, device='cuda:0', grad_fn= < DivBackward0 >) torch.Size([])
                    # tensor(2.7941, device='cuda:0', grad_fn= < AddBackward0 >) torch.Size([])
                    ...
                    logs.append(('MM', loss_CE_mm.item()))
                    logs.append(('aux', torch.mean(torch.sigmoid(aux_output)).item()))
                    logs.append(('MI', loss_MI.item()))
                    logs.append(('reg', reg_loss.item()))
        ...
        with torch.no_grad():
            ...
            if setting['get_MLP_score']>0:
                ...
                # vgg_correct,
                mm_correct,img_correct, text_correct,  ssim_correct = single_items
                # vgg_acc, vgg_correct[val_thresh],
                mm_acc, img_acc, text_acc,  ssim_acc = mm_correct[val_thresh], img_correct[val_thresh], text_correct[val_thresh],  ssim_correct[val_thresh]
                ...
                # VGG {vgg_acc}
                print(f'Single Modalities Accuracy:mm{mm_acc} Img {img_acc} Text {text_acc}  SSIM {ssim_acc}')
                print("------Real News -----------")
                ...
                if validate_acc > best_validate_acc:
                    ...
                    best_validate_dir = "{}/{}_dis/test_xai_1/{}_{}{}_{}.pkl".format(args.output_file,setting['train_dataname'],str(epoch + 1),str(datetime.datetime.now().month),str(datetime.datetime.now().day),
...
def evaluate(validate_loader, model, criterion, progbar=None, setting={}):
    ...
    img_correct, ssim_correct, text_correct, mm_correct = [0]*len(THRESH), [0]*len(THRESH), [0]*len(THRESH),  [0]*len(THRESH)
    ...
    for i, items in enumerate(validate_loader):
        ...
        # vgg_only_output,
        mix_output, mm_only_output,image_only_output, text_only_output,  aux_output, mi_loss, features = model(input_ids=input_ids,
                                                                        attention_mask=attention_mask,
                                                                        token_type_ids=token_type_ids,
                                                                        image=image,
                                                                        image_aug=image_aug,
        ...
        # vgg_feature,
        final_feature_main_task, shared_image_feature, shared_text_feature,  shared_mm_feature = features
        # yan:torch.cat会增加现有维度的值，可以理解为续接，torch.stack会新增加一个维度，可以理解为叠加
        ...
        val_mm_loss = criterion(mm_only_output, labels)
        ...
        if progbar is not None:
            ...
            logs.append(('mm_loss', val_mm_loss.item()))
            # logs.append(('mi_loss', val_mi_loss.item()))
            progbar.add(len(image), values=logs)

        # vgg_only_output, torch.sigmoid(vgg_only_output),
        mix_output, mm_only_output, image_only_output, text_only_output, aux_output = torch.sigmoid(mix_output),torch.sigmoid(mm_only_output), torch.sigmoid(
            image_only_output), torch.sigmoid(text_only_output),  torch.sigmoid(aux_output)

        for thresh_idx, thresh in enumerate(THRESH):
            # _, validate_argmax = torch.max(validate_outputs, 1)
            # yan:计算accuracy的话，需要先将概率转换成类别，这就需要手动设置一个阈值，
            # 如果对一个样本的预测概率高于这个预测，就把这个样本放进一个类别当中，
            # 如果低于这个阈值，就放在另一个类别当中，阈值在很大程度上影响了accuracy的计算
            ...
            validate_mm_argmax = torch.where(mm_only_output < thresh, 0, 1)
            ...
            y_pred_mm = validate_mm_argmax.squeeze().cpu().numpy()
            ...
            for idx, _ in enumerate(y_pred):
                if thresh_idx == 0:
                    ...
                    # record['mm_feature'] = mm_feature[idx].cpu().numpy().tolist()
                    ...
                    # record['y_pred_vgg']
                    record['y_pred_mm'], record['y_pred_img'], record['y_pred_text'],  = aux_output[idx].item(), \
                ...
                if y_pred_mm[idx] == y_GT[idx]: mm_correct[thresh_idx] += 1
    ...
    root_path = '/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/evaluate_txt_xlsx/dis'
    np.savetxt(f'{root_path}/npresultf.txt',resultf)
    np.savetxt(f'{root_path}/npresulti.txt',resulti)
    np.savetxt(f'{root_path}/npresultt.txt',resultt)
    # np.savetxt(f'{root_path}/npresultv.txt',resultv)
    np.savetxt(f'{root_path}/npresultm.txt',resultm)


    import pandas as pd
    df = pd.DataFrame(results)
    pandas_file = f'{root_path}/{dataset_name}_experiment.xlsx'
    ...
    for thresh_idx, _ in enumerate(THRESH):
        ...
        mm_correct[thresh_idx] = mm_correct[thresh_idx] / (realnews_sum[thresh_idx] + fakenews_sum[thresh_idx])
    ...
    return val_accuracy, (real_precision, real_recall, real_accuracy, real_F1),\
           ...
           val_loss,\
           (mm_correct,img_correct,text_correct,ssim_correct)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-output_file', type=str, default='/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/output_file', help='')
    ...
    print(args.output_file, os.path.exists(args.output_file),'='*100)
