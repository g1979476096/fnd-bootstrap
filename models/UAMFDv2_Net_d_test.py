...
class UAMFD_Net(nn.Module):
    def __init__(self,
        ...
        # yan: 对比自监督BYOL 以wim是否为零，把最终得到的五个特征之和视为两个结构相同参数不同的网络得出的结果
        self.projector_1 = nn.Sequential(nn.Linear(self.unified_dim, self.unified_dim, bias=False),
                                                    nn.BatchNorm1d(5),
                                                    nn.ReLU(),
                                                    nn.Linear(self.unified_dim, self.unified_dim, bias=False),
                                                    )
        self.projector_2 = nn.Sequential(nn.Linear(self.unified_dim, self.unified_dim, bias=False),
                                                    nn.BatchNorm1d(5),
                                                    nn.ReLU(),
                                                    nn.Linear(self.unified_dim, self.unified_dim, bias=False),
                                                    )
        self.predictor_1 = nn.Sequential(nn.Linear(self.unified_dim, self.unified_dim, bias=False),
                                                    nn.BatchNorm1d(5),
                                                    nn.ReLU(),
                                                    nn.Linear(self.unified_dim, self.unified_dim, bias=False),
                                                    )
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, image, no_ambiguity,\
        ...
        shared_intermm_feature_2 = shared_intermm_feature * 0
        ...
        concat_feature_main_biased_1 = torch.stack((shared_image_feature,
                                                  ...
                                                  # vgg_feature,
                                                  irrelevant_token,
                                                  shared_intermm_feature_2
                                                  ), dim=1)
        # yan  concat_feature_main_biased _1  为两个特征 进行projection
        # print(concat_feature_main_biased.shape,'ya*20') torch.Size([8, 5, 768]) ya*20
        output_view_1 = self.predictor_1(self.projector_1(concat_feature_main_biased))
        output_view_2 = self.projector_2(concat_feature_main_biased_1)

        # print(concat_feature_main_biased.shape,output_view_1.shape, output_view_2.shape, 'ya*20')
        # torch.Size([8, 5, 768]) torch.Size([8, 5, 768]) torch.Size([8, 5, 768]) ya*20
        ...
        if return_features:
            ...
            return mix_output, image_only_output, text_only_output, aux_output, \
                   mutual_info_loss,\
                   (final_feature_main_task_lite, shared_image_feature_lite, shared_text_feature_lite, shared_mm_feature_lite),(output_view_1,output_view_2)

        #  vgg_only_output,
        return mix_output, image_only_output, text_only_output, aux_output, \
               mutual_info_loss,(output_view_1,output_view_2)
               # torch.mean(self.irrelevant_tensor)
