...
class UAMFD_Net(nn.Module):
    def __init__(self,
        ...
        self.mm_alone_classifier = nn.Sequential(
            nn.Linear(64, out_dim),
        )
        ...
        self.mapping_MM_MLP_mu = nn.Sequential(
            nn.Linear(1, self.unified_dim),
            nn.SiLU(),
            # nn.BatchNorm1d(self.unified_dim),
            nn.Linear(self.unified_dim, 1),
        ...
        self.mapping_MM_MLP_sigma = nn.Sequential(
            nn.Linear(1, self.unified_dim),
            nn.SiLU(),
            # nn.BatchNorm1d(self.unified_dim),
            nn.Linear(self.unified_dim, 1),
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, image, no_ambiguity,\
                        ...
                        return_features=False,interpretability_img_s=None, interpretability_text_s=None, train_mode = 0):
        # 0513
        # train_mode 默认0:正常 , 1:EI模块detach 2:MR模块detach
        # 0513
        ...
        # 0418 加入 多模态分类检测
        shared_mm_feature_lite = self.mm_trim(shared_mm_feature)
        ...
        mm_only_output = self.mm_alone_classifier(shared_mm_feature_lite)
        ...
        mm_mu = self.mapping_MM_MLP_mu(torch.sigmoid(mm_only_output).clone().detach())
        ...
        mm_sigma = self.mapping_MM_MLP_sigma(torch.sigmoid(mm_only_output).clone().detach())
        ...
        irrelevant_token = self.adaIN(dis_irr,cc_mu+mm_mu,cc_sigma+mm_sigma)

        # irr_score = torch.ones_like(shared_mm_feature) * self.irrelevant_tensor  # torch.ones_like(shared_mm_feature).cuda()
        # other_irrelevant_token = self.adaIN(irr_score, cc_mu, cc_sigma)

        # print(irrelevant_token.shape, shared_intermm_feature.shape,'irr'*20)
        # yan: bsz 一半置零 为有监督数据   一般不置零为半监督数据
        # bsz 为 8
        shared_intermm_feature = shared_intermm_feature * 0.2
        # shared_intermm_feature = shared_intermm_feature * 0.1
        ...
        # shared_intermm_feature = shared_intermm_feature * 0
        #
        concat_feature_main_biased_1 = torch.stack((shared_image_feature,
                                                  shared_text_feature,
                                                  shared_mm_feature,
                                                  irrelevant_token,
                                                  ), dim=1)

        concat_feature_main_biased_2 = torch.stack((shared_intermm_feature,), dim=1)

        if train_mode == 1:
            concat_feature_main_biased_2 = concat_feature_main_biased_2.detach()
        elif train_mode == 2:
            concat_feature_main_biased_1 = concat_feature_main_biased_1.detach()


        concat_feature_main_biased = torch.cat((concat_feature_main_biased_1, concat_feature_main_biased_2), dim=1)
        ...
        if return_features:
            ...
            return mix_output, mm_only_output,image_only_output, text_only_output, aux_output, \
                   mutual_info_loss,\
