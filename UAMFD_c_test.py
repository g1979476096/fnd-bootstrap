...
def main(args):
    ...
    if setting['network_arch'] == 'UAMFDv2':
        from models.UAMFDv2_Net_b_test import UAMFD_Net
        ...
        if setting['val'] == 0:
            ...
            for i, items in enumerate(train_loader):
               ...
               (input_ids1, attention_mask1, token_type_ids1), (image1, labels1, sents1)
                    ...
                    # vgg_only_output,
                    mix_output, image_only_output, text_only_output, aux_output, mi_loss = model(input_ids=input_ids,
                    ...
                    if torch.isinf(reg_loss) or torch.isnan(reg_loss):
                        # reg_loss = torch.tensor(0.001,requires_grad=True)
                        reg_loss = torch.tensor(0)
                        # continue
                    #
                    loss = loss_CE+2.0*loss_ambiguity+1.0*loss_single_modal+0.01*loss_MI
                    # + reg_loss
                    # +1.0*loss_MI
                    # yan:
                    # print('=' * 20)
                    ...
                    # 每二次用reg_loss更新1次ig分支
                    if (epoch % 3 == 0) and reg_loss != 0:
...
def evaluate(validate_loader, model, criterion, progbar=None, setting={}):
    ...
    for i, items in enumerate(validate_loader):
        ...
        # vgg_only_output,
        mix_output, image_only_output, text_only_output,  aux_output, mi_loss, features = model(input_ids=input_ids,
                                                                        attention_mask=attention_mask,
                                                                        token_type_ids=token_type_ids,
                                                                        image=image,
                                                                        image_aug=image_aug,
