...
def main(args):
    ...
    if setting['network_arch'] == 'UAMFDv2':
        from models.UAMFDv2_Net_d_test import UAMFD_Net
    ...
    for epoch in range(custom_num_epochs):
        ...
        if setting['val'] == 0:
            ...
            for i, items in enumerate(train_loader):
               ...
               (input_ids1, attention_mask1, token_type_ids1), (image1, labels1, sents1)
                    ...
                    # vgg_only_output,
                    mix_output, image_only_output, text_only_output, aux_output, mi_loss, output_view = model(input_ids=input_ids,
                    ...
                    loss_MI  = mi_loss
                    # output_view1_detach = output_view[1].detach()
                    output_view_0 = nn.functional.normalize(output_view[0], p=2.0, dim=-1)
                    output_view_1 = nn.functional.normalize(output_view[1], p=2.0, dim=-1)
                    loss_mse = nn.MSELoss()(output_view_0,output_view_1)
                    ...
                    #
                    loss = loss_CE+2.0*loss_ambiguity+1.0*loss_single_modal+0.01*loss_MI + loss_mse
                    # + reg_loss
                    # +1.0*loss_MI
                    # yan:
                    # print('=' * 20)
        ...
        with torch.no_grad():
            ...
            if setting['get_MLP_score']>0:
                ...
                # vgg_correct,
                img_correct, text_correct,  ssim_correct = single_items
                # vgg_acc, vgg_correct[val_thresh],
                img_acc, text_acc,  ssim_acc = img_correct[val_thresh], text_correct[val_thresh],  ssim_correct[val_thresh]
                ...
                # VGG {vgg_acc}
                print(f'Single Modalities Accuracy: Img {img_acc} Text {text_acc}  SSIM {ssim_acc}')
                print("------Real News -----------")
...
def evaluate(validate_loader, model, criterion, progbar=None, setting={}):
    ...
    for i, items in enumerate(validate_loader):
        ...
        # vgg_only_output,
        mix_output, image_only_output, text_only_output,  aux_output, mi_loss, features, output_view = model(input_ids=input_ids,
                                                                        attention_mask=attention_mask,
                                                                        token_type_ids=token_type_ids,
                                                                        image=image,
                                                                        image_aug=image_aug,
        ...
        mix_output, image_only_output, text_only_output, aux_output = torch.sigmoid(mix_output), torch.sigmoid(
            ...
            for idx, _ in enumerate(y_pred):
                ...
                if y_GT[idx]==1:
                    ...
                    if y_pred[idx]==0:
                        ...
                        if thresh_idx == 0:
                            list_a = [0,0,0]
                            # list_a = [sents[idx],GT_path_origin,labels[idx].item()]
                            list_a[0] = sents[idx]
                            list_a[1] = GT_path_origin[idx]
                            list_a[2] = labels[idx].item()
                            print(list_a, '=a' * 20)
                            err_list.append(list_a)
                    else:
                    ...
                    if y_pred[idx]==1:
                        ...
                        if thresh_idx == 0:
                            list_b = [sents[idx],GT_path_origin[idx],labels[idx].item()]
                            print(list_b, '=b' * 20)
                            err_list.append(list_b)
                    else:
    ...
    import pandas as pd
    dfs = pd.DataFrame(err_list[1:], columns=err_list[0])
    dfs.to_csv('/home/yanwang_nuist/dev_workspace/prj_python/fnd-bootstrap/err_list_test.csv', index=False, sep=',')
    print('完成')
    # #y_GT
    tsnef = tsnef[1:,:]
    tsnei = tsnei[1:,:]
    ...
    return val_accuracy, (real_precision, real_recall, real_accuracy, real_F1),\
           ...
           val_loss,\
           (img_correct,text_correct,ssim_correct)
